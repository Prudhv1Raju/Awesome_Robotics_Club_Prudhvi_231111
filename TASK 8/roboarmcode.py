...
def constrain_angle(angle):
    # Constrain angle to the range [-pi/2, pi/2] (corresponding to -90 degrees to 90 degrees)
...
def fabrik(target, joint_positions, joint_lengths, tolerance=0.01, max_iterations=1000):
    ...
    while distance(joint_positions[-1], current_target) > tolerance and iterations < max_iterations:
        ...
        for i in range(num_joints - 2, -1, -1):
            ...
            # Constrain the angle of each joint
            angle = np.arctan2(link_direction[1], link_direction[0])
            joint_positions[i] = joint_positions[i+1] - link_lengths[i-1] * np.array([np.cos(constrain_angle(angle)), np.sin(constrain_angle(angle))])
